...
const commitmentTypes = [
  ...
  {
    value: "checkins_per_week",
    label: "Check-ins per Week",
    icon: Camera,
    color: "secondary",
  },
  {
    value: "meals_logged_per_week",
    label: "Meals Logged per Week",
    icon: Utensils,
    color: "accent",
  },
];
...
export const CommitmentsPage = ({ userId }: CommitmentsPageProps) => {
  ...
  const loadCommitments = async () => {
    try {
      ...
      const commitmentsWithProgress = await Promise.all(
        (data || []).map(async (commitment) => {
          const progress = await calculateProgress(commitment);
          return { ...commitment, progress };
        })
      );

      setCommitments(commitmentsWithProgress as Commitment[]);
    } catch (error) {
  ...
  const calculateProgress = async (commitment: any) => {
    const startDate = parseISO(commitment.start_date);
    const endDate = parseISO(commitment.end_date);
    const now = new Date();
    ...
    const totalWeeks = commitment.duration_weeks;
    ...
    // Get this week count based on type
    if (commitment.type === "workouts_per_week") {
    ...
    } else if (commitment.type === "checkins_per_week") {
      const { count } = await supabase
        .from("gym_checkins")
        .select("*", { count: "exact", head: true })
        .eq("user_id", userId)
    ...
    // Calculate overall progress (weeks completed successfully)
    ...
    for (let i = 0; i < currentWeek; i++) {
      ...
      if (commitment.type === "workouts_per_week") {
        const { count } = await supabase
          ...
          .select("*", { count: "exact", head: true })
          .eq("user_id", userId)
          .gte("workout_date", format(wStart, "yyyy-MM-dd"))
          .lte("workout_date", format(wEnd, "yyyy-MM-dd"));
        weekCount = count || 0;
      } else if (commitment.type === "checkins_per_week") {
        const { count } = await supabase
          .from("gym_checkins")
        ...
        const { count } = await supabase
          ...
          .select("*", { count: "exact", head: true })
          .eq("user_id", userId)
          .gte("meal_date", format(wStart, "yyyy-MM-dd"))
          .lte("meal_date", format(wEnd, "yyyy-MM-dd"));
        weekCount = count || 0;
      }

      weeklyResults.push(weekCount >= commitment.target_value);
    }

    const successfulWeeks = weeklyResults.filter(Boolean).length;
    const overallProgress = Math.round(
      (successfulWeeks / totalWeeks) * 100
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Set weekly targets and track your consistency.
          </p>
        </div>
      ...
      {commitments.length > 0 ? (
        <div className="grid gap-6 md:grid-cols-2">
          {commitments.map((commitment) => {
            ...
            const isOnTrack =
              progress && progress.thisWeekCount >= commitment.target_value;

            return (
              <Card
                key={commitment.id}
                className="p-6 border-border shadow-card relative"
              >
                <Button
                  variant="ghost"
                  size="icon"
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div
                    className={`w-10 h-10 rounded-full bg-${config.color}/10 flex items-center justify-center`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {commitment.target_value}x per week for{" "}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-muted/50 rounded-lg">
                      ...
                      <div className="flex items-center gap-3">
                        <Progress
                          value={
                            (progress.thisWeekCount / commitment.target_value) *
                            100
                          }
                          className="flex-1"
                        />
                      ...
                      {!isOnTrack && progress.thisWeekCount > 0 && (
                        <p className="text-xs text-muted-foreground mt-2">
                          {commitment.target_value - progress.thisWeekCount} more
                          to go this week!
                        </p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-1">
                        {Array.from({ length: progress.totalWeeks }).map(
                          (_, i) => {
        ...
        <Card className="p-8 text-center text-muted-foreground border-border shadow-card">
          ...
          <p className="text-sm mt-1">
            Create a commitment to stay accountable to your goals!
          </p>
        </Card>
      )}
    </div>
