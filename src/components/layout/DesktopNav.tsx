...
export const DesktopNav = ({ onSignOut }: DesktopNavProps) => {
  return (
    <header className="hidden md:block border-b border-border/60 bg-background/80 backdrop-blur-xl sticky top-0 z-50">
            ...
            <nav className="flex items-center gap-0.5">
              {navItems.map((item) => (
                <NavLink
                  key={item.to}
                  to={item.to}
                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-all whitespace-nowrap"
